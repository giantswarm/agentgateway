...
- changed: controller and proxy images follow the agentgateway line's first decoupled release `2.0.0` under the line's own nested names, `gsoci.azurecr.io/giantswarm/agentgateway-upstream/controller` and `…/agentgateway-upstream/agentgateway` (giantswarm/agentgateway-upstream, FORK.md "Publishing"). The line's versions are its own semver, decoupled from the vendored chart's version; which upstream pin a release carries is documented in the line's FORK.md (`main` @ `c1d24607` for `2.0.0`, the same build as `v1.5.1-gs.4`). The flattened `giantswarm/agentgateway` and `giantswarm/agentgateway-controller` hold copies of upstream's own releases only from now on. `make verify-sync` requires both tags to be one stable `X.Y.Z`; Renovate follows the line within `>=2.0.0 <3.0.0` (`versioning: semver`), and the `< 2.0.0` cap for the vendored charts excludes gsoci images explicitly.
- added: `sync/patches/image-tag` makes the vendored `agentgateway.imageTag` helper use the configured controller tag as-is. Upstream prepends a `v` to a bare semver tag because its own images carry one; the line's images are tagged bare `X.Y.Z`, so the unpatched helper rendered `controller.image.tag: 2.0.0` as `…/controller:v2.0.0`, a tag that does not exist. `make verify-sync` checks the patch is applied; the `.Chart.AppVersion` fallback passes through unchanged.
