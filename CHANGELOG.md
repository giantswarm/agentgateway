...
- fixed: the generated `values.schema.json` admits keys inside the chart's free-form objects: `controller.podDisruptionBudget`, `controller.strategy`, `controller.horizontalPodAutoscaler`, `controller.verticalPodAutoscaler`, `affinity`, `nodeSelector`, `commonLabels`, `podLabels`, `deploymentAnnotations`, `serviceAccount.annotations`, `controller.service.annotations`, `controller.service.extraLabels`, `controller.service.sessionAffinityConfig`, `controller.extraEnv`, `gatewayClassParametersRefs`, `monitoring.serviceMonitor.extraLabels` and `monitoring.proxy.namespaceSelector`. They are `{}` in `values.yaml`, and the schema generator (`noAdditionalProperties: true`) turned each into an object with no properties and `additionalProperties: false`, so any key set inside one failed the install: `helm template t . --set controller.podDisruptionBudget.maxUnavailable=1` stopped with `Error: values don't meet the specifications of the schema(s) in the following chart(s): - at '/controller/podDisruptionBudget': additional properties 'maxUnavailable' not allowed` (same for `controller.strategy.type`, `controller.verticalPodAutoscaler.updatePolicy`, `affinity.podAntiAffinity`, ...). Each carries a `# @schema additionalProperties: true` annotation now; the root and every typed object stay strict, so a typo in a key name still fails the install. `topologySpreadConstraints` and `dnsConfig` are declared in `values.yaml`: the deployment template already read them, but the strict root rejected both outright. `make verify-values-surface`, run by the `Verify vendored chart` workflow, renders `tests/values/surface.yaml` (a key inside every one of these objects plus the two new root keys), checks each value reaches its manifest, and checks an unknown key is still rejected.
