...
# because upstream's own images carry one (v1.5.0). The images this chart runs
# are releases of the Giant Swarm line of agentgateway, tagged bare X.Y.Z (the
# architect orb's convention; the line's FORK.md, "Publishing"): left alone,
# the helper renders controller.image.tag 2.0.0 as v2.0.0, a tag the line never
# publishes, and the controller pod cannot pull. Use the configured tag as-is.
# The .Chart.AppVersion fallback (upstream's release, with its v) passes
# through unchanged, so nothing changes for a chart that leaves the tag empty.
#
