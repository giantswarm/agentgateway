...
| topologySpreadConstraints | list | `[]` | Configure how controller pods are spread across topology domains, such as nodes or zones. |
...
| controller.image | object | `{"pullPolicy":"","registry":"gsoci.azurecr.io","repository":"giantswarm/agentgateway-controller","tag":"v1.5.1-gs.3"}` | Configure the controller container image. |
...
| controller.image.tag | string | `"v1.5.1-gs.3"` | Set the image tag for the controller. A release of the Giant Swarm line of agentgateway (github.com/giantswarm/agentgateway-upstream: the vendored upstream release rebuilt, scanned and signed there, mirrored into gsoci by the retagger); empty would fall back to .Chart.AppVersion, upstream's own build. renovate: datasource=docker depName=giantswarm/agentgateway-controller registryUrl=https://gsoci.azurecr.io |
...
| proxy | object | `{"image":{"registry":"gsoci.azurecr.io","repository":"giantswarm/agentgateway","tag":"v1.5.1-gs.3"}}` | Configure the agentgateway data plane deployment. |
...
| proxy.image.tag | string | `"v1.5.1-gs.3"` | Set the default image tag: the same release of the Giant Swarm line of agentgateway as the controller. renovate: datasource=docker depName=giantswarm/agentgateway registryUrl=https://gsoci.azurecr.io |
