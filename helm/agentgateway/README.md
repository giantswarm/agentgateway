...
| controller.image | object | `{"pullPolicy":"","registry":"gsoci.azurecr.io","repository":"giantswarm/agentgateway-upstream/controller","tag":"2.1.0"}` | Configure the controller container image. |
| controller.image.registry | string | `"gsoci.azurecr.io"` | Set the image registry for the controller. |
| controller.image.repository | string | `"giantswarm/agentgateway-upstream/controller"` | Set the image repository for the controller: the Giant Swarm line of agentgateway (github.com/giantswarm/agentgateway-upstream) publishes under its own nested path; the flattened giantswarm/agentgateway-controller holds copies of upstream's own builds. |
| controller.image.pullPolicy | string | `""` | Set the image pull policy for the controller. |
| controller.image.tag | string | `"2.1.0"` | Set the image tag for the controller. A release of the Giant Swarm line of agentgateway: the line's own semver, decoupled from the vendored chart's version (which upstream pin a release carries is documented in the line's FORK.md). Empty would fall back to .Chart.AppVersion, upstream's own build. renovate: datasource=docker depName=giantswarm/agentgateway-upstream/controller registryUrl=https://gsoci.azurecr.io |
...
| proxy | object | `{"image":{"registry":"gsoci.azurecr.io","repository":"giantswarm/agentgateway-upstream/agentgateway","tag":"2.1.0"}}` | Configure the agentgateway data plane deployment. |
| proxy.image.registry | string | `"gsoci.azurecr.io"` | Set the default image registry. Set to override the global value. |
| proxy.image.repository | string | `"giantswarm/agentgateway-upstream/agentgateway"` | Set the default image repository: the data-plane proxy of the Giant Swarm line of agentgateway, under the line's nested path. |
| proxy.image.tag | string | `"2.1.0"` | Set the default image tag: the same release of the Giant Swarm line of agentgateway as the controller. renovate: datasource=docker depName=giantswarm/agentgateway-upstream/agentgateway registryUrl=https://gsoci.azurecr.io |
...
| monitoring | object | `{"enabled":false,"grafanaDashboard":{"annotations":{},"enabled":true,"labels":{"grafana_dashboard":"1"}},"proxy":{"gatewayClassNames":["agentgateway"],"namespaceSelector":{},"podMonitor":{"enabled":true,"podTargetLabels":["gateway.networking.k8s.io/gateway-name"]}},"serviceMonitor":{"enabled":true,"extraLabels":{},"interval":"15s"}}` | Configure Prometheus and Grafana monitoring resources. |
...
| monitoring.serviceMonitor.extraLabels | object | `{}` | Additional labels to add to the controller ServiceMonitor and the proxy PodMonitor (e.g. release: prometheus).    The Giant Swarm observability platform routes a series to a Mimir tenant    by `observability.giantswarm.io/tenant`, which belongs here. |
...
| monitoring.proxy.podMonitor.podTargetLabels | list | `["gateway.networking.k8s.io/gateway-name"]` | Pod labels copied onto the scraped proxy series. The bundled Grafana    dashboard discovers and filters gateways by the gateway name, which    reaches the series only through this list.    Rendered by sync/patches/podmonitor-labels, a backport of    agentgateway#3421; both go at the bump to a release that carries it. |
...
| monitoring.grafanaDashboard.labels | object | `{"grafana_dashboard":"1"}` | Labels that the Grafana sidecar uses to discover dashboards. The Giant    Swarm observability platform discovers a dashboard ConfigMap by    `app.giantswarm.io/kind: dashboard`, which belongs here. |
| monitoring.grafanaDashboard.annotations | object | `{}` | Annotations on the dashboard ConfigMap. The Grafana organization and    folder the Giant Swarm observability platform loads the board into are    display names that contain a space, and no label value admits one, so    `observability.giantswarm.io/organization` and `.../folder` belong here.    Rendered by sync/patches/dashboard-annotations, filed upstream as    agentgateway#3591; both go at the bump to a release that carries it. |
